...
void behv_drift(void *ref)
{
  ...
  if(unit->active)
  {
    ...
    if(psyh)
    {
      float mov_x = psyh->vel_x * (context->tick_delta / 100000);
      float mov_y = psyh->vel_y * (context->tick_delta / 100000);

      if(psyh->pos_x + mov_x < 0 || psyh->pos_x + mov_x > context->win_x)
      {
        psyh->vel_x = 0 - psyh->vel_x;
        mov_x = psyh->vel_x * (context->tick_delta / 100000);
      }
      if(psyh->pos_y + mov_y < 0 || psyh->pos_y + mov_y > context->win_y)
      {
        psyh->vel_y = 0 - psyh->vel_y;
        mov_y = psyh->vel_y * (context->tick_delta / 100000);
      }
